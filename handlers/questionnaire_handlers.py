...
async def process_start_command(message: Message, state: FSMContext):
    await message.answer(LEXICON[message.text], reply_markup=keyboards.keyboard_no_yes)
    await state.set_state(FSMFillForm.is_ready_questionnaire)


# Обработка ответа на вопрос заполнения анкеты
@router.message(
    StateFilter(FSMFillForm.is_ready_questionnaire), F.text == LEXICON_BUTTON["yes"]
)
async def process_yes_questionnaire(message: Message, state: FSMContext):
    await message.answer(LEXICON["enter_name"], reply_markup=ReplyKeyboardRemove())
    await state.set_state(FSMFillForm.fill_name)
    await state.update_data(chat_id=message.chat.id)


@router.message(
    StateFilter(FSMFillForm.is_ready_questionnaire), F.text == LEXICON_BUTTON["no"]
)
async def process_no_questionnaire(message: Message, state: FSMContext):
    await message.answer(LEXICON["wait"], reply_markup=ReplyKeyboardRemove())
    await state.clear()


@router.message(StateFilter(FSMFillForm.is_ready_questionnaire))
async def process_other_questionnaire(message: Message):
    await message.answer(LEXICON["use_btn_pls"], reply_markup=keyboards.keyboard_no_yes)
...
async def process_old(message: Message, state: FSMContext):
    await message.answer(
        LEXICON["enter_gender"], reply_markup=keyboards.keyboard_gender
    )
...
@router.message(
    ...
    F.text.in_([LEXICON_BUTTON["male"], LEXICON_BUTTON["female"]]),
)
...
async def process_gender_error(message: Message):
    await message.answer(LEXICON["use_btn_pls"], reply_markup=keyboards.keyboard_gender)
...
async def process_weight(message: Message, state: FSMContext):
    ...
    await message.answer(
        create_questionnaire_text(await state.get_data()),
...
# Верна ли анкета
@router.message(
    StateFilter(FSMFillForm.is_correct_questionnaire), F.text == LEXICON_BUTTON["yes"]
)
...
@router.message(
    StateFilter(FSMFillForm.is_correct_questionnaire), F.text == LEXICON_BUTTON["no"]
)
async def process_no_correct_que(message: Message, state: FSMContext):
    await message.answer(
        LEXICON["questionnaire_again"], reply_markup=ReplyKeyboardRemove()
    )
